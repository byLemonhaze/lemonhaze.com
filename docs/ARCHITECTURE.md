...
|---|---|---|
| Frontend | Vite app (`index.html`, `src/`) | Collection browsing, deep links, modals, internal sections, blog, collector lookup |
| Secondary entry | `supply.html` + `src/supply.js` | Separate supply-oriented page kept as its own Vite input |
...
| Edge API | Cloudflare Pages Functions (`functions/api`) | Inscription metadata proxying and password-protected Press Engine generation |
...
The router in `src/router/index.js` owns query-based state:
...
Legacy keys are still accepted and normalized. Invalid or conflicting combinations are rewritten to the canonical URL shape.

### 3. Views

- Collection browsing is coordinated by `collection-flow.js`.
- Internal sections (`about`, `highlights`, `supply`, `media`, `collectors`, `blog`, `projects`) are coordinated by `section-flow.js`.

...
test('Supply uses the complete 48-collection Ord.net roster without a Provenance collection', () => {
    ...
    assert.deepEqual(rowsByName.get('Manufactured'), {
        name: 'Manufactured', year: 2024, inscribed: 428, circulating: 233,
    });
    ...
    assert.deepEqual(totals, { inscribed: 1643, circulating: 1296 });
    assert.equal(totals.inscribed - totals.circulating, 347);
});

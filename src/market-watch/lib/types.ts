...
export function deduplicate(results:Result[]){const byId=new Map<string,{id:string;name:string;observations:(Listing&{market:Market;key:string;checkedAt:string})[]}>();for(const r of results){if(r.status==='error'||r.status==='missing')continue;for(const l of r.listings){const id=l.id.toLowerCase();if(!/^[a-f0-9]{64}i\d+$/.test(id))continue;let group=byId.get(id);if(!group){group={id,name:l.name,observations:[]};byId.set(id,group)}if(!group.observations.some(o=>o.market===r.market))group.observations.push({...l,id,market:r.market,key:r.key,checkedAt:r.checkedAt});}}return [...byId.values()];}

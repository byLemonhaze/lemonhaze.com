import {canonicalCatalog,canonicalSnapshot} from './collection-policy';
...
export function createStore(db:D1Database){
 ...
 async function snapshot():Promise<Snapshot>{const rows=await db.prepare('SELECT key,value FROM market_watch_snapshots').all<{key:string;value:string}>();const s=structuredClone(initial) as Snapshot;for(const row of rows.results){const v=JSON.parse(row.value);if(row.key==='catalog')Object.assign(s,v);else if(row.key.startsWith('result:'))s.results[row.key.slice(7)]=v;}return canonicalSnapshot(s);}
 async function catalog(){return canonicalCatalog((await get<{catalog:Collection[]}>('catalog'))?.catalog||(initial as Snapshot).catalog);}
 async function result(key:string,market:string){return await get<Result>(`result:${market}:${key}`)||(await snapshot()).results[`${market}:${key}`]||null;}
